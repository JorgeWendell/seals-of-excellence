...
import heroImg from "@/assets/hero-seals2.png";
...
export function HomePage() {
  usePageMeta({
    ...
    description:
      "Há 41 anos fabricando selos mecânicos de alta performance para a indústria. Soluções confiáveis, eficientes e sob medida para cada aplicação.",
  });
    ...
    <div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-10 items-center py-16 lg:py-24">
          <div className="relative z-10">
            ...
            <p className="mt-6 text-base sm:text-lg text-muted-foreground max-w-xl">
              ...
              Soluções confiáveis, eficientes e sob medida para cada aplicação. Há 41 anos ao lado
            ...
            <div className="mt-12 grid grid-cols-2 sm:grid-cols-4 gap-6">
              {features.map((f) => (
                <div key={f.title} className="flex flex-col items-start">
                  ...
                  <div className="mt-3 font-display font-bold text-sm text-foreground">
                    {f.title}
                  </div>
          ...
          <div className="relative">
            <div
              className="absolute -inset-6 bg-gradient-to-tr from-primary/10 to-accent-red/10 blur-3xl rounded-full"
              aria-hidden
            />
            <img
              src={heroImg}
      ...
      <section className="py-10 bg-secondary border-y border-border">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-center gap-6 text-center sm:text-left">
          <img
            src={duolap}
            alt="Du-O-Lap Selos Mecânicos"
            className="h-20 w-20 object-contain"
            width={80}
            height={80}
          />
          <div>
            <div className="font-display uppercase tracking-[0.25em] text-xs text-muted-foreground font-semibold">
              Revendedor Autorizado
            </div>
            <div className="font-display text-2xl sm:text-3xl font-bold text-[#1f7a1f] uppercase">
              Du-O-Lap Selos Mecânicos
            </div>
            <p className="text-sm text-muted-foreground mt-1 max-w-xl">
              Distribuímos a linha completa Du-O-Lap com garantia de fábrica e suporte técnico
              oficial.
            </p>
          </div>
        </div>
      ...
      <section className="py-20 bg-background">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <Link
              to="/empresa"
              className="mt-6 inline-block font-display font-semibold uppercase tracking-wider text-sm text-primary hover:text-accent-red transition"
            >
              Conheça a empresa →
            </Link>
          </div>
